...
FIXED_MODEL_PARAMETERS="--bert_model ${BERT_MODEL} --train_batch_size 8 --num_train_epochs 4 --learning_rate 0.00005"

python3 train.py ${FIXED_DATA_PARAMETERS} ${FIXED_MODEL_PARAMETERS}

python3 train.py ${FIXED_DATA_PARAMETERS} ${FIXED_MODEL_PARAMETERS} --use_crf

python3 train.py ${FIXED_DATA_PARAMETERS} ${FIXED_MODEL_PARAMETERS} --extra_classifier_features

python3 train.py ${FIXED_DATA_PARAMETERS} ${FIXED_MODEL_PARAMETERS} --extra_classifier_features --use_crf


FIXED_DATA_PARAMETERS="--labels_file ${LABEL_FILE} --dataset_split ${DATASET_SPLIT} --num_splits ${NUM_SPLITS} --fp16"
FIXED_MODEL_PARAMETERS="--bert_model ${BERT_MODEL} --train_batch_size 8 --num_train_epochs 10 --classifier_lr 0.001"
